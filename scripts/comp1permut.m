...
for i = 1:340
    ...
    'sub1 ',grp{grpIndx(1)},' ', conds{condIndx(1,1)},' AviMa0/1/SAM_1_40Hz/wordSingleComp1+tlrc '...
    'sub1 ',grp{grpIndx(1)},' ', conds{condIndx(2,1)},' AviMa0/1/SAM_1_40Hz/nonWordSingleComp1+tlrc '...
    'sub2 ',grp{grpIndx(2)},' ', conds{condIndx(1,2)},' AviMa1/1/SAM_1_40Hz/wordSingleComp1+tlrc '...
    'sub2 ',grp{grpIndx(2)},' ', conds{condIndx(2,2)},' AviMa1/1/SAM_1_40Hz/nonWordSingleComp1+tlrc '...
...
for i=1:1000
    % compute volume of largest positive clusters
    eval(['!~/abin/3dclust -quiet -1clip 7.942 5 125 comp1premut',num2str(i),'_ctx+tlrc''[1]''+tlrc > posClustGrp_01.txt'])
    eval(['!~/abin/3dclust -quiet -1clip 4.089 5 125 comp1premut',num2str(i),'_ctx+tlrc''[2]''+tlrc > posClustSem_01.txt'])
    eval(['!~/abin/3dclust -quiet -1clip 4.089 5 125 comp1premut',num2str(i),'_ctx+tlrc''[3]''+tlrc > posClustInt_01.txt'])
    posClustGrp=importdata('posClustGrp_01.txt');
    posClustSem=importdata('posClustSem_01.txt');
    posClustInt=importdata('posClustInt_01.txt');
    if iscell(posClustGrp)
        posClustGrpSize=0;
    else
        posClustGrpSize=posClustGrp(1)/125;
...
save comp1permutCrit_Size_F_01 clustSizeCritGrp clustSizeCritSem clustSizeCritInt

%% permutation on t-test statist for comp4Grp voxels (randomizing the group)
subs=1:24;
data=[Clust20_conVoxelsComp4Grp;Clust20_szVoxelsComp4Grp];
T=zeros(1000,1);
for i=1:1000
    randSubs=subs(randperm(length(subs)));
    con=data(randSubs(1:14),:);
    sz=data(randSubs(15:24),:);
    [H(i,1:9),P(i,1:9),CI,STATS]=ttest2(con,sz);
    T(i,1:9)=STATS.tstat;
    disp(i);
end
...
cd /home/meg/Data/Maor/PhD_SAM/AllSubsSAMfiles/MVMpermutsComp4
save t_threshold H P T thresh